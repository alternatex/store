...
var ApiClient = $hxClasses["ApiClient"] = function() { }
ApiClient.__name__ = ["ApiClient"];
ApiClient.url = null;
ApiClient.cnx = null;
ApiClient.proxy = null;
ApiClient.main = function() {
	ApiClient.url = "index.php";
	ApiClient.cnx = haxe.remoting.HttpAsyncConnection.urlConnect(ApiClient.url);
	ApiClient.proxy = new ApiProxy(ApiClient.cnx.resolve("api"));
	ApiClient.proxy.test(3,5,ApiClient.display);
}
ApiClient.display = function(result) {
	console.log("The server calculated 3 + 5 to equal: " + result);
}
